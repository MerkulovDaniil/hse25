...
* $\alpha_k = \dfrac{\gamma}{\|g_k\|_2}$ -- constant step length ($\| x_{k+1}  - x_k\| = g_k$)

* $\alpha_k: \sum_{k=1}^{\infty}a_k^2<\infty$ and $\sum_{k=1}^{\infty}a_k=\infty$ -- square summable but not summable

* $\alpha_k: \lim_{k\rightarrow\infty}\alpha_k=0$ and $\sum_{k=1}^{\infty}a_k=\infty$ -- nonsummable diminishing
...
\end{cases}
$$
and check out the sample code here [\faPython](https://colab.research.google.com/github/MerkulovDaniil/hse25/blob/main/notebooks/s13_svm_hingeloss.ipynb).
:::
