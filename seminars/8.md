...
$$
where $f(x)$ is convex and $L$-smooth. Find convergence rate of gradient descent with constant step size $\eta$ for the \textit{mean point} and for the \textit{best point}. 
...
:::{.callout-tip}
## Tip
Use the fact that $\Psi_k(x)$ is $\frac{1}{\eta}$-strongly convex due to quadratic regularizer.
:::
