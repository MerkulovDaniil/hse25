...
## Intuition on how to improve Newton's method
...
$$
    f(y) \le f(x) + \left\langle\nabla f(x), y - x\right\rangle + \frac{1}{2} \left\langle\nabla^{2} f(x)(y - x), y - x\right\rangle + \frac{M}{6} \left\|y - x\right\|^{3}.
$$
Minimizing the right-hand side of this inequality, we come to Cubic-regularized Newton's method
...
1. We can't get explicit expression for $x_{k + 1}$ (without argmin) from \eqref{eq:cubreg}.
...
2. This problem is equivalent to a convex one-dimensional optimization problem.
:::

## Convergence \footnote{\href{https://arxiv.org/pdf/2410.04083}{Kamzolov, Dmitry, et al. "Optami: Global superlinear convergence of high-order methods". Accepted to ICLR 2025.}}
